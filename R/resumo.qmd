...
A @fig-densidade apresenta a estimativa de densidades do número de eleitores aptos para o pleito de 2018 dentro das seções que foram com e sem biometria. Nota-se, para esse pleito, que há uma concentração de seções com número de eleitores aptos superior a 300 nas seções 100% biometricas. Ainda, há um número expressivo de seções com mais de 400 eleitores. Esse fato pode estar associado as agregações que são realizadas durante a eleição. Nas seções com votação sem identificação biométrica, a maior densidade fica em torno de 370.

De maneira complementar, são apresentadas algumas medidas descritivas para o eleitorado apto na votação 100% biométrica na @tbl-resumo_estat. O número médio de eleitores aptos foi cerca de 304 eleitores com um desvio padrão de cerca de 66 eleitores. O valor mediano foi de 367, de modo que 50% das seções eleitorais no pleito de 2018 tinham valores de eleitorado apto igual ou superior a esse valor. Na @fig-bp_secoes.

```{r}
#| fig-cap: "Densidade dos aptos por seção com relação à votação com e sem biometria em 2018"
#| fig-align: center
#| label: fig-densidade
